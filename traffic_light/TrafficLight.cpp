...
TrafficLight::TrafficLight() : color(LightColor::RED), running(false) {}
...
void TrafficLight::start() {
    ...
    lightFuture = std::async(std::launch::async, &TrafficLight::run, this);
}
...
void TrafficLight::waitForGreen() {
    ...
    while (color != LightColor::GREEN) {
        cv.wait(lock);  // Wait and re-check
    }
}
